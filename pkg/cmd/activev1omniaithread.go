...
var activeV1OmniAIThreadsGetThread = cli.Command{
	Name:    "get-thread",
	Usage:   "Returns metadata (title, timestamps) for a single thread. Does not include\nmessages — use `GET /omni-ai/threads/{thread_id}/messages` for conversation\nhistory.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:     "thread-id",
