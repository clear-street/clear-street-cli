...
var v1OmniAIThreadsGetThread = cli.Command{
	Name:    "get-thread",
	...
	Action:          handleV1OmniAIThreadsGetThread,
	HideHelpCommand: true,
}

var v1OmniAIThreadsListThreads = cli.Command{
	Name:    "list-threads",
	...
	Action:          handleV1OmniAIThreadsListThreads,
	HideHelpCommand: true,
}

var v1OmniAIThreadsResponse = cli.Command{
	Name:    "response",
	...
	Action:          handleV1OmniAIThreadsResponse,
	HideHelpCommand: true,
}
...
func handleV1OmniAIThreadsGetThread(ctx context.Context, cmd *cli.Command) error {
	client := clearstreet.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := clearstreet.V1OmniAIThreadGetThreadParams{}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "v1:omni-ai:threads get-thread",
		Transform:      transform,
	})
}

func handleV1OmniAIThreadsListThreads(ctx context.Context, cmd *cli.Command) error {
	client := clearstreet.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := clearstreet.V1OmniAIThreadListThreadsParams{}

	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.V1.OmniAI.Threads.ListThreads(ctx, params, options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "v1:omni-ai:threads list-threads",
		Transform:      transform,
	})
}

func handleV1OmniAIThreadsResponse(ctx context.Context, cmd *cli.Command) error {
	client := clearstreet.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	if !cmd.IsSet("thread-id") && len(unusedArgs) > 0 {
		cmd.Set("thread-id", unusedArgs[0])
	...
	params := clearstreet.V1OmniAIThreadResponseParams{}
	...
	return ShowJSON(obj, ShowJSONOpts{
		...
		Format:         format,
		RawOutput:      cmd.Root().Bool("raw-output"),
		Title:          "v1:omni-ai:threads response",
		Transform:      transform,
	})
}
